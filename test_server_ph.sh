...
IPS="8.212.155.150 8.220.151.101 47.242.77.178 8.212.166.134 8.218.3.248 8.212.155.30"
best_ip=""
...
for ip in $IPS; do
    echo "Testing $ip ..."
    result=$(curl -s -w --connect-timeout 2  "%{http_code} %{time_total}" -o /tmp/curl_$ip.txt "http://$ip:8080/api/copyright")
